  ...
  kthFromEnd(k){
    if(typeof k === 'number'){
      if(this.head.next === null){
        return '__Error__ The linked list has a length of 1, there is no kth from the end to search for.';
      }else if(k > 0){
        return '__Error__ The number provided is not a positive number!';
      } else {
        let acc = 0;
        let curr = this.head;
        while(curr.next){
